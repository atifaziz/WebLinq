...
namespace WebLinq
{
    ...
    {
        public static IEnumerable<Vars> Vars() =>
            from s in new[] { new VarService() }
            select s.Vars;

        public static IEnumerable<object> Var(string name) =>
            Var<object>(name);

        public static IEnumerable<T> Var<T>(string name) =>
        ...
        public static IEnumerable<TResult> Var<T, TResult>(string name, Func<T, TResult> selector) =>
            from a in Var<T>(name)
            select selector(a);

        public static IEnumerable<TResult> Vars<T1, T2, TResult>(string name1, string name2, Func<T1, T2, TResult> selector) =>
            from a in Var<T1>(name1)
            from b in Var<T2>(name2)
            select selector(a, b);

        public static IEnumerable<TResult> Vars<T1, T2, T3, TResult>(string name1, string name2, string name3, Func<T1, T2, T3, TResult> selector) =>
            from a in Var<T1>(name1)
            from b in Var<T2>(name2)
            from c in Var<T3>(name3)
            select selector(a, b, c);

        public static IEnumerable<TResult> Var<T, TResult>(Func<T, TResult> selector) =>
            selector.GetParameterNames().GetEnumerator().Map(ns =>
                from a in Var<T>(ns.Read())
                select selector(a));

        public static IEnumerable<TResult> Vars<T1, T2, TResult>(Func<T1, T2, TResult> selector) =>
            selector.GetParameterNames().GetEnumerator().Map(ns =>
                from a in Var<T1>(ns.Read())
                from b in Var<T2>(ns.Read())
                select selector(a, b));

        public static IEnumerable<TResult> Vars<T1, T2, T3, TResult>(Func<T1, T2, T3, TResult> selector) =>
            selector.GetParameterNames().GetEnumerator().Map(ns =>
                from a in Var<T1>(ns.Read())
                from b in Var<T2>(ns.Read())
        ...
        public static IEnumerable<T> Var<T>(string name, T value) =>
            from vars in Vars()
            select (T) vars[name];

        public static IEnumerable<T> Swap<T>(string name, T value) =>
