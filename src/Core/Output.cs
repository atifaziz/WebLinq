...
namespace WebLinq
{
    ...
    public sealed class CsvOutputFormatBuilder<T>
    {
        ...
        internal CsvOutputFormatBuilder(IObservable<T> query)
        {
        ...
        public IObservable<string> Lines(bool includeHeaders = false) =>
            !HasFields
            ? Reflect().Lines(includeHeaders)
            : _query.Select(e => string.Join(",", Fields.Select(f => Csv.EncodeField(f.Value(e)))))
                    .StartWith(Enumerable.Repeat(string.Join(",", Fields.Select(f => Csv.EncodeField(f.Key))), 1))
                    .Skip(includeHeaders ? 0 : 1);
    }

    public static class CsvOutputFormat
    {
        public static IObservable<string> ToCsv<T>(this IObservable<T> query) =>
            new CsvOutputFormatBuilder<T>(query).Lines();

        public static IObservable<ICsvOutputFormatPartial> Partial =
            Observable.Return(CsvOutputFormatPartial.Singleton);
    }

    public interface ICsvOutputFormatPartial
    {
        CsvOutputFormatBuilder<T> Source<T>(IObservable<T> query);
    }
    ...
    sealed class CsvOutputFormatPartial : ICsvOutputFormatPartial
    {
        ...
        public CsvOutputFormatBuilder<T> Source<T>(IObservable<T> query) => new CsvOutputFormatBuilder<T>(query);
