...
namespace WebLinq
{
    ...
    public interface IHttpObservable<out T> : IObservable<T>
    {
        IObservable<IHttpClient> HttpClient { get; }
    }

    public static class HttpQuery
    {
        sealed class HttpObservable<T> : IHttpObservable<T>
        {
            readonly IObservable<T> _results;

            public HttpObservable(IObservable<IHttpClient> httpClient, IObservable<T> results)
            {
                _results = results;
            ...
            public IObservable<IHttpClient> HttpClient { get; }
        }

        static class HttpObservable
        {
            public static IHttpObservable<T> Create<T>(IObservable<IHttpClient> httpClient, IObservable<T> results) =>
                new HttpObservable<T>(httpClient, results);
        }

        static HttpFetch<HttpContent> Send(IHttpClient http, int id, HttpMethod method, Uri url, HttpContent content = null, HttpOptions options = null)
        {
            ...
            return http.Send(request, options).ToHttpFetch(id);
        }

        public static IHttpObservable<HttpFetch<HttpContent>> Get(this IHttpObservable<HttpFetch<HttpContent>> http, Uri url) =>
            http.HttpClient.Get(url, null);

        public static IHttpObservable<HttpFetch<HttpContent>> Get(
                this IHttpObservable<HttpFetch<HttpContent>> http, Uri url, HttpOptions options) =>
            http.HttpClient.Get(url, options);

        public static IHttpObservable<HttpFetch<HttpContent>> Get(this IObservable<IHttpClient> http, Uri url) =>
            http.Get(url, null);

        public static IHttpObservable<HttpFetch<HttpContent>> Get(this IObservable<IHttpClient> http, Uri url, HttpOptions options) =>
            HttpObservable.Create(http,
                from client in http
                select Send(client, 0, HttpMethod.Get, url, options: options));

        public static IHttpObservable<HttpFetch<HttpContent>> Post(this IObservable<IHttpClient> http, Uri url, NameValueCollection data) =>
        ...
        public static IHttpObservable<HttpFetch<HttpContent>> Post(this IObservable<IHttpClient> http, Uri url, HttpContent content) =>
            HttpObservable.Create(http,
                from client in http
                select Send(client, 0, HttpMethod.Post, url, content));
        ...
        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IHttpObservable<HttpFetch<HttpContent>> query, string formSelector, NameValueCollection data) =>
            query.Submit(query.HttpClient, formSelector, data);

        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IHttpObservable<HttpFetch<HttpContent>> query, int formIndex, NameValueCollection data) =>
            query.Submit(query.HttpClient, formIndex, data);

        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IObservable<HttpFetch<HttpContent>> query, IObservable<IHttpClient> http, string formSelector, NameValueCollection data) =>
            query.Submit(http, formSelector, null, data);

        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IObservable<HttpFetch<HttpContent>> query, IObservable<IHttpClient> http, int formIndex, NameValueCollection data) =>
            query.Submit(http, null, formIndex, data);

        static IHttpObservable<HttpFetch<HttpContent>> Submit(this IObservable<HttpFetch<HttpContent>> query, IObservable<IHttpClient> http, string formSelector, int? formIndex, NameValueCollection data) =>
            HttpObservable.Create(http,
                from html in query.Html()
                from fetch in Submit(http, html.Content, formSelector, formIndex, data)
                select fetch);

        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IObservable<IHttpClient> http, ParsedHtml html, string formSelector, NameValueCollection data) =>
            Submit(http, html, formSelector, null, data);

        public static IHttpObservable<HttpFetch<HttpContent>> Submit(this IObservable<IHttpClient> http, ParsedHtml html, int formIndex, NameValueCollection data) =>
            Submit(http, html, null, formIndex, data);

        static IHttpObservable<HttpFetch<HttpContent>> Submit(IObservable<IHttpClient> http, ParsedHtml html,
